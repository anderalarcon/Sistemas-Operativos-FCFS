...
Libro *agregarLibro(Libro*Lista,int *ord){
...
int main()
{
    int i=0,n,m,ni=0,mi=1,numero,numero1,j,k,opcion,frecuencia,temp1,tecla,error=0;
    ...
    printf("\n\n");
    FILE *flujo;
    flujo = fopen("BLOC 1.4.txt","rb");
    if(flujo==NULL){
        perror("Error en apertura del archivo");
        return 1;
    }
    Libro *Lista = listaLibro(Lista);
    while(feof(flujo)==0 && i!=n){
        fscanf(flujo,"%d-%d",&numero,&numero1);
        p[i].tllegada=numero;
        p[i].tcpu=numero1;
        Lista=agregarLibro(Lista,mi);
        i++;
        mi++;
        ni++;
    }
    ...
    while(tecla!=5){
        ...
        if (tecla==1){
            ...
            printf("\n");
            for (i=0;i<n;i++){
                printf("%c %cProceso[%i]  \t%c",179,250,orden[i],179);
                for (j=0;j<mi;j++){
                    if (i==0){
        ...
        printf("\n");
        for(i=0 ; i<n ; i++){
            printf("%c %cProceso[%i] \t%c     [%i] \t%c\t[%i] \t    %c        [%i] \t %c        [%i]    \t%c\n",179,250,orden[i],179,p[i].tllegada,179,p[i].tcpu,179,p[i].tespera,179,p[i].tretorno,179);
        }
        ei_izq();
        linea(15);
        medio_bajo();
